...
// 处理图片 URL：相对路径加上 host 前缀
const formatImageUrl = (url) => {
  if (!url) return '/static/placeholders/cover-rect.png';
  if (url.startsWith('http')) return url; // 已经是完整 URL
  return `${serverHost}${url}`; // 拼接完整 URL
};

const categories = [
  { name: '全部', type: '' },
  { name: '茶类分类', type: 'tea_category'},
  { name: '冲泡教程', type: 'brewing' },
  { name: '产地文化', type: 'origin_culture' }
];
  ...
  try {
    ...
    if (!currentType.value) {
      newList = newList.filter(item => 
        ['tea_category', 'brewing', 'origin_culture'].includes(item.type)
      );
    }
